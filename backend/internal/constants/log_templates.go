...
const (
	...
	LogProjectCreate       = "project create by=%s title=%s interviewee=%s status=%s tag_count=%d"
	LogProjectUpdate       = "project update by=%s project=%d title=%s status=%s"
	LogProjectTagsUpdate   = "project tags replace by=%s project=%d tag_count=%d"
	LogProjectListSearch   = "project list query page=%d pageSize=%d status=%s tag_count=%d keyword=%q"
